...
Interrogation reads `openai-completions`, `openai-responses`, and `anthropic-messages`: the `GET /models` shape with bearer auth is the one a gateway, a self-hosted server, and the official endpoints all agree on, and a gateway speaking Anthropic for chat still exposes that listing. Azure is excluded despite its OpenAI lineage — it authenticates with an `api-key` header and requires an `api-version` query — and Codex uses OAuth; every other protocol answers `DISCOVERY_UNSUPPORTED` so the surface falls back to hand-entry instead of an authentication failure being reported as a provider with no models. Who owns the version segment differs by SDK convention — OpenAI SDKs fold it into the base, Anthropic's appends it — so a `baseURL` without one is probed at `/models` and then `/v1/models`, one already carrying it is probed as-is, and an `anthropic-messages` route's `baseURL` has a trailing `/v1` stripped before the Anthropic SDK would append its own. The `baseURL` is otherwise treated as a prefix rather than a URL to resolve against, so a deployment path such as `https://gateway.example/openai/v1` keeps its segments.

Most listings disclose an id and nothing else; `context_window`/`context_length` and `max_output_tokens`/`max_tokens` are read when a gateway supplies them, entries without a usable id are skipped rather than failing the whole listing, and everything else the adopting surface still owes. The reply is read under a four-megabyte ceiling enforced on the bytes actually received — the endpoint is a URL the user typed, so a declared length is checked first but never trusted as the bound. An unreachable endpoint, a refused credential, a non-JSON body, and a body with no `data` array all fail with `DISCOVERY_FAILED` and a message naming the endpoint — the versioned path's verdict, when both candidates were asked and failed — and, for a 401 or 403 alone, the credential. Cancellation during the body read surfaces as `ABORTED`, like a cancellation before the request went out.
